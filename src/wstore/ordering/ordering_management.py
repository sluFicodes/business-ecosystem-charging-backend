...
class OrderingManager:
    ...
    def notify_item_completed(self, order_model: Order, contract: Contract, raw_order):
        ...
        if settings.PAYMENT_METHOD != "dpas":
            try:
                ...
                elif record.payment_type == "redsys":
                    attr_name = "redsysPreAuthorizationId"
            except Exception:
                pass
        for sale_id in order_model.sales_ids:
    ...
    def activate_product(self, order_id, product):
        ...
        inventory_client = InventoryClient()
        activation_date = inventory_client.activate_product(product["id"])

        # TODO: Update acbr period
        # acbrs = billing_client.get_acbrs_by_cb(contract.product_id, contract.customer_bill)
        # new_acbr_period = ...
        # billing_client.set_acbrs_new_period...