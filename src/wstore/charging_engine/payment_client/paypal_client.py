...
PAYPAL_CLIENT_ID = os.environ.get(
    ...
    "AVOLRuc4jN599UD5FMLHv07T7pnmh76zrllx60cQ-fPK39Bu4yR2iOCzNrzqou6XmAFbnuYhdMY4cExY",
)
PAYPAL_CLIENT_SECRET = os.environ.get(
    "BAE_CB_PAYPAL_CLIENT_SECRET",
    "EAgeaOxAgJ5ZMMu9Tf4riICdT7Sz2y77PRBwUIYppNlf_xw2Q0WD1_jCG4YzSLNxQFevkNnFovtT02u7",
)
    ...
    def start_redirection_payment(self, transactions):
        ...
        payment = paypalrestsdk.Payment(
            {
                ...
                "transactions": [
                    {
                        "amount": {"total": str(t["price"]), "currency": t["currency"]},
        ...
        if not payment.create():
            ...
            if (
                len(transactions) > 1
                and len(details) == 1
                and details[0]["issue"] == "Only single payment transaction currently supported"
            ):
